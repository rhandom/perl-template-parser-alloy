...
#process_ok('[% [0..10].${ 2.3 } %]' => '2') if ! $is_tt; # TT2 engine doesn't support non-integer indexes
...
#process_ok("[% n|format('%0*d', 3) %]" => '007', {n => 7}) if ! $is_tt; # TT2 doesn't support the extra parameter
...
#process_ok("[% n.repeat(2,'|') %]" => '1|1', {n => 1}) if ! $is_tt; # TT2 doesn't allow passing extra item to join on
...
process_ok("[% n.split.join('|') %]" => "a|b|c", {n => "a b c"});
#process_ok("[% n.split(u,2).join('|') %]" => "a|b c", {n => "a b c", u => undef}) if ! $is_tt;  # TT2 has different split behavior
process_ok("[% n.split(u,2).join('|') %]" => "a| b c", {n => "a b c", u => undef});#  if $is_tt; # TT2 has different split behavior
process_ok("[% n.split('/').join('|') %]" => "a|b|c", {n => "a/b/c"});
...
#process_ok("[% a.import(5) %]|[% a.join %]" => '|2 3', {a => [2,3]}) if ! $is_tt;                 # TT2 has different import behavior
process_ok("[% a.import(5) %]|[% a.join %]" => qr{^ARRAY.+|2 3$ }x, {a => [2,3]}); # if $is_tt;    # TT2 has different import behavior
#process_ok("[% a.import([5]) %]|[% a.join %]" => '|2 3 5', {a => [2,3]}) if ! $is_tt;             # TT2 has different import behavior
process_ok("[% a.import([5]) %]|[% a.join %]" => qr{ARRAY.+|2 3 5$ }x, {a => [2,3]}); # if $is_tt; # TT2 has different import behavior
...
#process_ok("[% h.item('_a') %]" => '', {h => {_a => 'A'}}) if ! $is_tt; # TT2 has a bug that allows access to "hidden" items
...
#process_ok("[% n|size %]", => '2',    {n => {size => 'SIZE', a => 'A'}}) if ! $is_tt; # TT2 has no size filter
...
#process_ok("[% n FILTER repeat(2,'|') %]" => '1|1', {n => 1}) if ! $is_tt; # TT2 doesn't support item to join on
...
#process_ok('[% 1|format("%*s", 6) %]' => '     1') if ! $is_tt;  # TT2 doesn't support extra *
#process_ok('[% 1|format("%-*s", 6) %]' => '1     ') if ! $is_tt; # TT2 doesn't support extra *
...
#process_ok("[% f.b = 1 %][% FOREACH f.b = [1..10] %][% f.b %][% END %][% f.b %]" => '1234567891010') if ! $is_tt; # TT2 doesn't support nested keys
process_ok("[% a = 1 %][% FOREACH [{a=>'A'},{a=>'B'}] %]bar[% a %][% END %][% a %]" => 'barAbarB1');
process_ok("[% FOREACH [1..3] %][% loop.size %][% END %][% loop.size %]" => '333');
#process_ok("[% FOREACH i = [1..3] %][% loop.size %][% END %][% loop.size %]" => '333') if ! $is_tt; # TT2 has different loop initialize behavior
process_ok("[% FOREACH i = [1..3] %][% loop.size %][% END %][% loop.size %]" => '3331');# if $is_tt; # TT2 has different loop initialize behavior
...
#process_ok("[% TRY %]Foo[% THROW foo 'for fun' %]bar[% CATCH %][% END %]hi" => 'Foohi') if ! $is_tt; # TT2 doesn't like empty catch blocks
...
# TT2 doesn't have recursion protection
...
#process_ok("[% constants.foo.\$bam.\${constants.bing} %]" => '42', {tt_config => [@config_c]}) if ! $is_tt; # TT2 doesn't support this
...
# TT2 doesn't have eval recursion protection
